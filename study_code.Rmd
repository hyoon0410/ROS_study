...
```{r}
wells <- here("ROS-Examples-master", "Arsenic", "data", "wells.csv")

wells<- read.csv(wells, header = TRUE)

fit_4 <- brm(switch ~ dist100 + arsenic + dist100:arsenic,
  family=bernoulli(link="logit"), data=wells)

fit_4_stan <- stan_glm(switch ~ dist100 + arsenic + dist100:arsenic,
...
```{r}
fit_5 <- brm(switch ~ c_dist100 + c_arsenic + c_dist100:c_arsenic,
     family=bernoulli(link="logit"), data=wells)

fit_5_stan <- stan_glm(switch ~ c_dist100 + c_arsenic + c_dist100:c_arsenic,
     family=binomial(link="logit"), data=wells)
```
...
```{r}
fit <- brm(switch ~ dist100, family=bernoulli(link="logit"), data=wells)
fit_stan <- stan_glm(switch ~ dist100, family=binomial(link="logit"), data=wells)

sims <- as.matrix(fit)
sims_stan <- as.matrix(fit_stan)

n_sims <- nrow(sims)
n_sims_stan <-nrow(sims_stan)

plot(sims[,1], sims[,2], xlab=expression(beta[0]), ylab=expression(beta[1]))

plot(wells$dist100, wells$y)
   for(s in 1:20){
     curve(invlogit(sims[s,1] + sims[s,2]*x), col="red", lwd=1, add=TRUE)
   }
...
```{r}
fit_7 <- brm(switch ~ dist100 + arsenic + educ4, family=bernoulli(link="logit"),
     data=wells)

fit_7_stan <- stan_glm(switch ~ dist100 + arsenic + educ4, family=binomial(link="logit"),
     data=wells)
```
...
fit_fake <- brm(y ~ x, family=poisson(link="log"), data=fake)
print(fit_fake)

plot(x, y)
curve(exp(fixef(fit_fake)[1] + fixef(fit_fake)[2]*x), add=TRUE)
```
...
for (k in 1:K){
 ...
 fit_nb[[k]] <- brm(y ~ x, family=neg_binomial_2(link="log"), data=fake)
 print(fit_nb[[k]])
}

for (k in 1:K) {
 plot(x, y_nb[[k]])
 curve(exp(fixef(fit_nb[[k]])[1] + fixef(fit_nb[[k]])[2]*x), add=TRUE)
}
```

```{r}
roaches$roach100 <- roaches$roach1/100
# fit_1 <- brm(y ~ roach100 + treatment + senior, family=negbinomial(),
#                   offset=log(n), data=roaches)
...
test_rep_2 <- apply(y_rep_2, 1, test)
```

#Example of ordered categorical regression
```{r}
Storable <- here("ROS-Examples-master", "Storable", "data", "mesquite.dat")

Storable<- read.csv(Storable, header = TRUE)
```

#Mixed discrete/continuous data
```{r}
fit_1a <- stan_glm((earn > 0) ~ height + male, family=binomial(link="logit"),
     data=earnings)
